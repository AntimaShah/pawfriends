...
router.post("/volunteer", async (req, res) => {
  const { name,age, email,address, phone,contribute,availability, password, cpassword } = req.body;
  try {
...
router.post("/service", async (req,res)=>{
  try {
    const { servicer } = req.body; // Destructure servicer from req.body
    const service = new Service({ servicer }); // Create a new Service instance with servicer property
...
router.post("/sterilization", async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: "sterilization" });
